...
fn send_request(
    ...
    serde_json::from_str(&response_line).unwrap_or_else(|_| {
        serde_json::json!({"error": "Failed to parse response", "raw": response_line})
    })
}
...
fn test_mcp_initialization() {
    ...
    let binary_path = format!("{}/target/release/mcp-wasm-host", env!("CARGO_MANIFEST_DIR"));
    ...
    assert!(response.get("result").is_some(), "Initialize should return result: {:?}", response);
...
fn test_list_tools() {
    ...
    assert!(response.get("result").is_some(), "Should have result: {:?}", response);
    ...
    assert!(tool_names.contains(&"register_script"), "Should have register_script");
...
fn test_code_validation() {
    ...
    assert!(content.as_str().unwrap().contains("passed") || content.as_str().unwrap().contains("✓"));
